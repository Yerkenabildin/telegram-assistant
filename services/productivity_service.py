...
Кратко (1-2 предложения) опиши что я сделал/чего достиг в этом чате. Фокусируйся на результатах, а не на процессе.
Например: "Провёл ревью и замержил PR", "Починил баг с авторизацией", "Согласовал архитектуру нового сервиса".
НЕ пиши "обсуждал", "работал над", "занимался" — пиши что именно было сделано/решено.
Если из сообщений нельзя точно определить результат — опиши ближайшее к результату действие, но не выдумывай.
Отвечай от первого лица."""
    ...
    def _generate_keyword_summary(self, chat: ChatSummary) -> str:
        ...
        # Achievement patterns (result-oriented labels)
        patterns = [
            (r'\b(ревью|review|pr|пр|merge)\b', 'провёл ревью'),
            (r'\b(баг|bug|фикс|fix|исправ)\b', 'исправил баги'),
            (r'\b(созвон|звонок|call|митинг|meeting)\b', 'провёл созвон'),
            (r'\b(задач|task|тикет|ticket|jira)\b', 'разобрал задачи'),
            (r'\b(деплой|deploy|релиз|release)\b', 'выкатил деплой/релиз'),
            (r'\b(вопрос|помо[гщ]|подска)\b', 'помог с вопросами'),
            (r'\b(тест|test)\b', 'написал/прогнал тесты'),
            (r'\b(документ|doc|readme)\b', 'обновил документацию'),
        ]
        ...
        if detected:
            return ', '.join(detected[:3]).capitalize()
    ...
    async def _generate_overall_insights(
        ...
        prompt = f"""Проанализируй мою активность за день и выдели конкретные достижения/результаты:

{context}

Всего: {daily.total_messages} сообщений в {daily.total_chats} чатах.

Напиши 2-4 пункта списком — что я сделал/чего достиг за день.
Фокусируйся на результатах, а не на процессе. Не пиши "обсуждал", "работал над" — пиши что было сделано, решено, завершено.
Если точный результат неизвестен — опиши ближайшее к результату действие, но не выдумывай.
Формат:
- Пункт 1
- Пункт 2
